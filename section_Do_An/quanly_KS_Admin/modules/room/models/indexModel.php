...
//    return $result;
    if(!empty($where))
        $where="WHERE {$where}";
    $list_rooms = db_fetch_array("SELECT room.id, room.roomNumber,room.image,room.price,roomtype.name,room.description,room.state FROM `room`,`roomtype` {$where} LIMIT {$start},{$num_per_page}");
    return $list_rooms;
}
